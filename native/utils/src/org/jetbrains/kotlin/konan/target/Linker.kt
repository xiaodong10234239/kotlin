...
class HMOSLinker(private val targetProperties: HMOSConfigurables) : LinkerFlags(targetProperties), HMOSConfigurables by targetProperties {

    private val toolchainPath = "$absoluteTargetToolchain/bin"
    ...
    override fun LinkerArguments.finalLinkCommands(): List<Command> {
        ...
        if (kind == LinkerOutputKind.STATIC_LIBRARY)
            return staticGnuArCommands(ar, executable, objectFiles, libraries)
        val dynamic = kind == LinkerOutputKind.DYNAMIC_LIBRARY
        val toolchainSysroot = targetProperties.absoluteTargetSysRoot
        val clangTarget = HMOS.architectureDirForTarget(target)
        val libDirs = listOf(
            "--sysroot=$toolchainSysroot", "-L$toolchainSysroot/usr/lib/${clangTarget}"
        )
        return listOf(Command("${toolchainPath}/clang++").apply {
            +"-o"
            +executable
            when (kind) {
                LinkerOutputKind.EXECUTABLE -> +listOf("-fPIE", "-pie")
...
fun linker(configurables: Configurables): LinkerFlags =
        when (configurables) {
            ...
            is HMOSConfigurables -> HMOSLinker(configurables)
