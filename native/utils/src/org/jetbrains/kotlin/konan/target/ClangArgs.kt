...
internal object HMOS {

    private val architectureMap = mapOf(
        KonanTarget.HARMONYOS_X64 to "x86_64",
        KonanTarget.HARMONYOS_ARM64 to "aarch64"
    )

    fun architectureDirForTarget(target: KonanTarget) =
        "${architectureMap.getValue(target)}-linux-ohos"
}
...
sealed class ClangArgs(
    ...
    private val commonClangArgs: List<String> = mutableListOf<List<String>>().apply {
        ...
        val hasCustomSysroot = configurables is ZephyrConfigurables
                ...
                || configurables is HMOSConfigurables
                || argsForWindowsJni
        if (!hasCustomSysroot) {
            when (configurables) {
    ...
    private val specificClangArgs: List<String> = when (target) {
       ...
       KonanTarget.WATCHOS_ARM32 -> listOf(
        ...
        KonanTarget.HARMONYOS_X64, KonanTarget.HARMONYOS_ARM64 -> {
            val architectureDir = HMOS.architectureDirForTarget(target)
            val toolchainSysroot = configurables.absoluteTargetSysRoot
            listOf(
                "--target=$architectureDir",
                "--sysroot=$toolchainSysroot",
                "-I$absoluteTargetToolchain/include/libcxx-ohos/include/c++/v1",
                "-I$toolchainSysroot/usr/include",
                "-I$toolchainSysroot/usr/include/$architectureDir"
            )
        }
