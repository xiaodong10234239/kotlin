...
interface KotlinTargetContainerWithPresetFunctions : KotlinTargetsContainerWithPresets {
    ...
    fun harmonyOSArm64(
        name: String = "harmonyOSArm64",
        configure: KotlinNativeTarget.() -> Unit = { }
    ): KotlinNativeTarget =
        configureOrCreate(
            name,
            @Suppress("DEPRECATION_ERROR")
            presets.getByName("harmonyOSArm64") as KotlinNativeTargetPreset,
            configure
        )

    fun harmonyOSArm64() = harmonyOSArm64("harmonyOSArm64") { }
    fun harmonyOSArm64(name: String) = harmonyOSArm64(name) { }
    fun harmonyOSArm64(name: String, configure: Action<KotlinNativeTarget>) = harmonyOSArm64(name) { configure.execute(this) }
    fun harmonyOSArm64(configure: Action<KotlinNativeTarget>) = harmonyOSArm64 { configure.execute(this) }

    fun harmonyOSX64(
        name: String = "harmonyOSX64",
        configure: KotlinNativeTarget.() -> Unit = { }
    ): KotlinNativeTarget =
        configureOrCreate(
            name,
            @Suppress("DEPRECATION_ERROR")
            presets.getByName("harmonyOSX64") as KotlinNativeTargetPreset,
            configure
        )

    fun harmonyOSX64() = harmonyOSX64("harmonyOSX64") { }
    fun harmonyOSX64(name: String) = harmonyOSX64(name) { }
    fun harmonyOSX64(name: String, configure: Action<KotlinNativeTarget>) = harmonyOSX64(name) { configure.execute(this) }
    fun harmonyOSX64(configure: Action<KotlinNativeTarget>) = harmonyOSX64 { configure.execute(this) }
